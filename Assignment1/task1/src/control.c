...
{
 if (argc < 2)                                                            /*Too few arguments specified.*/
 {
 ...
 {
  printf("WARNING: Disregarding %d trailing arguments",argc - 2);         /*Warn the user that excess arguments have will be disregarded.*/
 }
 ...
 struct_argpasser.seconds = *argv[1];                                     /*Maximum amount of seconds the thread should run.*/
 ...
 struct_argpasser.max_iterations = *argv[0];                              /*Maximum amount of iterations the thread should run.*/
 ...
 returncode = pthread_create(&pth_calculator,&thread_attributes,maartensfunction,&struct_argpasser);/*Creat the thread that calculates π.*/
 ...
 while(busy)                                                              /*Check whether the calculation thread is still running.*/
 {
  ...
  printf("/n");                                                           /*Print the newline character.*/
 }
 int status;                                                              /*The variable that will hold the pthread_join status.*/
 pthread_join(pth_calculator,&status);                                    /*Wait for the calculation thread to exit*/
 printf("Joined with calculation thread with status: %d",status);         /*Alert the user to the status of the join with the calculation thread.*/
 int i;                                                                   /*Counter variable*/
 printf("Pi: %x.",pi[0]);                                                 /*Print the first digit of π and the separator.*/
 for (i = 1; i < iteration; i++)                                          /*Loop through the digits stored in the array.*/
 {
